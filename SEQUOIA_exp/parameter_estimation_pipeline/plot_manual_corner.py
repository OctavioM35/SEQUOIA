...
def plot_manual_corner(folder, zenodo_file, event_dir, outdir,dicc):
        if dicc["inference"]["approximant"] == False:
            ...
            OUTPUT_PLOT = os.path.join(outdir, "DANSur_vs_Zenodo_corner_comparison_" + folder + '.png' )

        else:
            bilby_result = os.path.join(outdir, "IMR_result.json")
            OUTPUT_PLOT = os.path.join(outdir, "IMR_vs_Zenodo_corner_comparison_" + folder + '.png' )
